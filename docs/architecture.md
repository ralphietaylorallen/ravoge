...
`/coach/install` and `/client/install` are public, role-specific PWA entry routes. They carry no trusted role or organization state. General links route existing members through normal login. A secure invitation URL contains only a cryptographically random, expiring opaque token; `/enroll/[role]` resolves the authoritative database invitation, places the token in a short-lived `Secure`, `HttpOnly`, `SameSite=Lax` server handoff, and redirects to a clean URL with no token. Token-bearing responses are private/no-store, noindex, and use `Referrer-Policy: no-referrer`.

The PWA manifest launches at `/launch`, uses standalone display mode and Ravoge branding, and exposes Coach and Client shortcuts. The launch handler rechecks the Auth session and active membership. When an exact-email invitation is still pending it calls the atomic invitation-acceptance function, consumes the browser handoff, and redirects to the database-authoritative dashboard. Existing accounts receive a locked-email login; new invitees receive the matching locked-email signup. Used, expired, revoked, malformed, wrong-role, and wrong-email attempts grant nothing. The service worker intentionally caches no application, authentication, or enrollment response.
