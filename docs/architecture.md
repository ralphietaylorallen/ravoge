...
`/coach/install` and `/client/install` are public, role-specific PWA entry routes. Organization enrollment begins with a high-entropy opaque bearer token. The database stores only its SHA-256 digest, fixed organization, fixed Coach or Client role, lifecycle state, creator, timestamps, and optional expiry. The application can redisplay a reusable Owner QR without storing raw tokens by deterministically deriving it from the credential id and rotation nonce with the server-only `RAVOGE_ENROLLMENT_TOKEN_KEY`. Rotation changes the nonce and digest atomically, invalidating old printed QRs; disable blocks only future enrollment. The token contains no readable organization data, password, JWT, session, or API key.

After `/enroll/[role]` validates the server-resolved role, it sets a short-lived `Secure`, `HttpOnly`, `SameSite=Lax` convenience handoff and sends the device to the install screen. A token-specific, private/no-store manifest launches the installed PWA at `/launch?enrollment=…`, preserving the opaque context across Safari/browser installation even if browser cookies are partitioned. `/launch` immediately converts that query token back to the protected handoff and removes it from the visible URL. Existing accounts sign in; new accounts supply full name, email, and password. Acceptance rechecks the Auth identity and active credential in one server-side transaction, then creates only the stored organization/role membership. A same membership is idempotent; inactive or conflicting organization/role membership is rejected. Email-bound invitations use the same acceptance function and retain their exact-email and one-time constraints.
...
Local calendar-day decisions—including workout due status, booking defaults, prescription dates, closure filtering, and progress week boundaries—use the organization's IANA timezone. `America/Denver` is the explicit fallback for legacy rows only. The `save_organization_schedule_settings` database function derives the active Owner organization from `auth.uid()`, validates every field before mutation, locks the membership/organization, and writes the organization, seven weekly hour rows, and session settings in one transaction.
