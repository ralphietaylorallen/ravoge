...
export default async function ClientBookPage({ searchParams }: { searchParams: Promise<{ date?: string; duration?: string; reschedule?: string }> }) {
  ...
  const [{ data: profile }, { data: organization }, { data: coaches }, { data: settings }] = await Promise.all([
    ...
    supabase.rpc("get_my_assigned_coach"),
  ...
  return <DashboardShell compact gymName={organization?.name ?? "Ravoge gym"} name={clientName} role="client">
