...
import { DEFAULT_ORGANIZATION_TIMEZONE, localDateInTimeZone, localDateStartIso, shiftLocalDate } from "@/lib/timezone";
...
export default async function OwnerSchedulePage({ searchParams }: { searchParams: Promise<{ coach?: string; client?: string; status?: string }> }) {
  const { coach: filterCoach, client: filterClient, status: filterStatus } = await searchParams;
  const { membership, supabase, userId } = await requireRole("owner");
  const organizationId = membership.organization_id;
  const [{ data: owner }, { data: organization }, { data: hours }, { data: settings }, { data: coachMembers }, { data: clientMembers }, { data: coachAvailability }] = await Promise.all([
    ...
    supabase.from("organization_memberships").select("user_id").eq("organization_id", organizationId).eq("role", "client").eq("status", "active"),
  ...
  const windowStart = localDateStartIso(today, timezone)!;
  const windowEnd = localDateStartIso(shiftLocalDate(today, 31)!, timezone)!;
  let bookingQuery = supabase.from("bookings").select("id,coach_user_id,client_user_id,starts_at,ends_at,timezone,status").eq("organization_id", organizationId).gte("starts_at", windowStart).lt("starts_at", windowEnd).order("starts_at").limit(500);
  if (filterCoach) bookingQuery = bookingQuery.eq("coach_user_id", filterCoach);
  if (filterClient) bookingQuery = bookingQuery.eq("client_user_id", filterClient);
  if (filterStatus && ["scheduled", "completed", "cancelled"].includes(filterStatus)) bookingQuery = bookingQuery.eq("status", filterStatus);
  const { data: bookingRows } = await bookingQuery;
  const ids = [...new Set([...(coachMembers ?? []).map((row: { user_id: string }) => row.user_id), ...(clientMembers ?? []).map((row: { user_id: string }) => row.user_id), ...(bookingRows ?? []).flatMap((row: { coach_user_id: string; client_user_id: string }) => [row.coach_user_id, row.client_user_id])])];
  ...
  return <DashboardShell gymName={organization?.name ?? "Ravoge gym"} name={owner?.full_name ?? "Owner"} role="owner">
    ...
    <section className={styles.workspaceSection}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{today} → {shiftLocalDate(today, 30)}</p><h3>All sessions</h3></div></div><form className={styles.bookingFilters}><label className={styles.field}>Coach<select defaultValue={filterCoach ?? ""} name="coach"><option value="">All Coaches</option>{(coachMembers ?? []).map((row: { user_id: string }) => <option key={row.user_id} value={row.user_id}>{profileMap.get(row.user_id)?.preferred_name || profileMap.get(row.user_id)?.full_name || "Coach"}</option>)}</select></label><label className={styles.field}>Client<select defaultValue={filterClient ?? ""} name="client"><option value="">All Clients</option>{(clientMembers ?? []).map((row: { user_id: string }) => <option key={row.user_id} value={row.user_id}>{profileMap.get(row.user_id)?.preferred_name || profileMap.get(row.user_id)?.full_name || "Client"}</option>)}</select></label><label className={styles.field}>Status<select defaultValue={filterStatus ?? ""} name="status"><option value="">All</option><option value="scheduled">Scheduled</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></label><button className={styles.secondaryAction} type="submit">Apply</button></form><BookingList bookings={bookings} empty="No sessions match this 30-day view." perspective="owner" staffControls /></section>
