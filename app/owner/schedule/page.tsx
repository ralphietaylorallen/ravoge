...
export default async function OwnerSchedulePage({ searchParams }: { searchParams: Promise<{ coach?: string }> }) {
  ...
  const [{ data: owner }, { data: organization }, { data: hours }, { data: settings }, { data: closures }, { data: coachMembers }, { data: coachAvailability }] = await Promise.all([
    ...
    supabase.from("organization_special_hours").select("id,local_date,label").eq("organization_id", organizationId).gte("local_date", new Date().toISOString().slice(0,10)).order("local_date"),
  ...
  return <DashboardShell gymName={organization?.name ?? "Ravoge gym"} name={owner?.full_name ?? "Owner"} role="owner">
    ...
    <section className={styles.workspaceSection}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Gym hours</p><h3>Scheduling rules</h3></div><p>Standard hours only. Overnight hours are intentionally not supported in beta.</p></div><OrganizationScheduleForm address={organization?.address} hours={hours ?? []} settings={settings ?? defaults} timezone={organization?.timezone ?? "America/Denver"} /></section>
    <section className={styles.workspaceSection}><div className={styles.detailGrid}><div className={styles.panel}><h2>Date-specific closures</h2><ClosureForm /></div><div className={styles.panel}><h2>Upcoming closures</h2>{closures?.length ? <ul className={styles.list}>{closures.map((closure: { id: string; label: string; local_date: string }) => <li key={closure.id}><span>{closure.local_date} · {closure.label}</span><form action={deleteClosureAction.bind(null, closure.id)}><button className={styles.textAction} type="submit">Remove</button></form></li>)}</ul> : <p className={styles.empty}>No upcoming closures.</p>}</div></div></section>
  </DashboardShell>;
}
