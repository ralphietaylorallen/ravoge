...
export default async function CoachSchedulePage() {
  ...
  return <DashboardShell gymName={organization?.name ?? "Ravoge gym"} name={coachName} role="coach"><div className={styles.pageHeading}><div><p className={styles.eyebrow}>Coach schedule</p><h2>Sessions</h2></div><p>Your authorized appointments. Each opens the corresponding client profile.</p></div><section className={styles.panel}><h2>Today</h2><BookingList bookings={todays} empty="No sessions today." perspective="coach" staffControls /></section><section className={styles.workspaceSection}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Week &amp; upcoming</p><h3>Upcoming sessions</h3></div></div><BookingList bookings={bookings.filter((booking) => Date.parse(booking.startsAt) >= currentTime)} empty="No upcoming sessions." perspective="coach" staffControls /></section></DashboardShell>;
}
