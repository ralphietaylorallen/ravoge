...
-- Owner organization provisioning uses the authenticated identity and a
-- constrained request; browser metadata cannot choose a different identity.
reset role;
set local role authenticated;
...
insert into public.organization_creation_requests (requested_by, organization_name)
values ('10000000-0000-0000-0000-000000000005', 'New Owner Gym');
do $$ begin
  if not exists (
    select 1
